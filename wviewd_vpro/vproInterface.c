	...
	void (*archiveIndication)(ARCHIVE_PKT *newRecord))
{
	...
	if (!strcmp(work->stationInterface, "serial"))
	{
		...
		{
			MsgLog(PRI_HIGH, "stationInit: serial MediumInit failed");
			return ERROR;
		}
	}
	...
	{
		MsgLog(PRI_HIGH, "stationInit: medium setup failed");
		return ERROR;
	}
	...
	if ((int)work->archiveDateTime == ERROR)
	{
		...
		MsgLog(PRI_STATUS, "stationInit: no archive records found in database!");
	}
	...
	{
		MsgLog(PRI_HIGH, "stationInit: radStatesInit failed");
...
int stationGetPosition(WVIEWD_WORK *work)
{
	...
	{
		MsgLog(PRI_HIGH, "stationGetPosition: WAKEUP failed");
		return ERROR;
	}
	...
	{
		MsgLog(PRI_HIGH, "stationGetPosition: failed");
		return ERROR;
	}
	else
	{
...
static void convertToArchivePkt(WVIEWD_WORK *work, ARCHIVE_RECORD *newRecord, ARCHIVE_PKT *archivePkt)
{
	...
	archivePkt->value[DATA_INDEX_rainRate] = (float)newRecord->highRainRate / vpWorkData.rainTicksPerInch;
	archivePkt->value[DATA_INDEX_rain] = ((float)(newRecord->rain & 0xFFF)) / vpWorkData.rainTicksPerInch;
	...
	{
		archivePkt->value[DATA_INDEX_extraTemp2] = wvutilsCalculateApparentTemp((float)archivePkt->value[DATA_INDEX_outTemp], (float)archivePkt->value[DATA_INDEX_windSpeed], (float)archivePkt->value[DATA_INDEX_outHumidity]);
	}
	...
	//{
	//    archivePkt->value[DATA_INDEX_extraTemp1] = ( float )( newRecord->extraTemp1);
	//}
...
static void storeLoopPkt(WVIEWD_WORK *work, LOOP_DATA *src, LOOP2_DATA *src2, int retVal1, int retVal2)
{
	...
	if (tempshort != 0x7FFF && tempshort != 0xFFFF && tempshort <= 4000)
	...
	dest->extraTemp[2] = (float)(src->extraTemp2 - 90);
	dest->extraTemp[3] = (float)(src->extraTemp3 - 90);
	if (src->soilTemp1 != 0xFF)
		dest->soilTemp1 = (float)(src->soilTemp1 - 90);
	if (src->soilMoist1 != 0xFF)
		dest->soilMoist1 = src->soilMoist1;
	if (src->leafWet1 != 0xFF)
		dest->leafWet1 = src->leafWet1;
	if (src->outHumidity != 0xFF && src->outTemp != 0x7FFF && src->tenMinuteAvgWindSpeed != 0xFFFF)
	{
...
static int wakeupConsole(WVIEWD_WORK *work)
{
	...
	while (retries > 0)
	{
		...
		if (retVal != 1)
		{
			MsgLog(PRI_MEDIUM, "wakeupConsole: Write ERROR!");
			return ERROR;
		}
		...
		{
			// MsgLog (PRI_MEDIUM, "wakeupConsole: bad read: retVal=%d", retVal);
		}
	...
	MsgLog(PRI_MEDIUM, "wakeupConsole: failed");
...
int vpifSynchronizeConsoleClock(WVIEWD_WORK *work)
{
	...
	*/
	if (vpifWakeupConsole(work) == ERROR)
	{
		MsgLog(PRI_HIGH, "vpifSynchronizeConsoleClock: WAKEUP1 failed");
		return ERROR;
	}
	...
	*/
	...
	{
		MsgLog(PRI_HIGH, "vpifSynchronizeConsoleClock: GMT failed");
		return ERROR;
	}
	...
	*/
	if (vpifWakeupConsole(work) == ERROR)
	{
		MsgLog(PRI_HIGH, "vpifSynchronizeConsoleClock: WAKEUP2 failed");
		return ERROR;
	}
...
int vpifSendLoopRqst(WVIEWD_WORK *work, int number)
{
	...
	sprintf((char *)ptr, "LPS 3 %d", 2 * number);
...
int vpifGetRXCheck(WVIEWD_WORK *work)
{
	...
	//MsgLog(PRI_MEDIUM, "RXCHECK: %s", vpWorkData.rxCheck);
	return OK;
}
