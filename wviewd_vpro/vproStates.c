...
int vproStartProcState(int state, void* stimulus, void* data)
{
	...
	case STIM_DUMMY:
		...
		{
			MsgLog(PRI_HIGH, "vproStartProcState: WAKEUP failed - retry");
			radUtilsSleep(1000);
			numTries++;
		}
		if (numTries == WVD_INITIAL_WAKEUP_TRIES)
		{
			MsgLog(PRI_HIGH, "vproStartProcState: WAKEUP failed - ERROR");
		...
		{
			MsgLog(PRI_HIGH, "vproStartProcState: vpifGetArchiveInterval failed");
			return VPRO_STATE_ERROR;
		}
		...
		{
			MsgLog(PRI_HIGH, "vproStartProcState: stationVerifyArchiveInterval failed!");
			MsgLog(PRI_HIGH, "You must either move old /var/wview/archive files out of the way -or-");
			MsgLog(PRI_HIGH, "fix the station setting...");
			radProcessEventsSend(NULL, STATION_INIT_COMPLETE_EVENT, 1);
			return VPRO_STATE_ERROR;
		}
		else
		{
			MsgLog(PRI_STATUS, "station archive interval: %d minutes",
		...
		{
			MsgLog(PRI_HIGH, "vproStartProcState: vpifGetRainCollectorSize failed");
			return VPRO_STATE_ERROR;
		}
		else
		{
			MsgLog(PRI_STATUS, "station rain ticks/inch: %.0f",
		...
		{
			MsgLog(PRI_HIGH, "vproStartProcState: WAKEUP2 failed - ERROR");
			return VPRO_STATE_ERROR;
		}
		...
		if (work->stationGeneratesArchives)
		{
			...
			{
				MsgLog(PRI_HIGH, "vproStartProcState: DMPAFT_RQST failed");
				return VPRO_STATE_ERROR;
			}
		...
		else
		{
			...
			{
				MsgLog(PRI_HIGH, "vproStartProcState: LOOP_RQST failed");
				return VPRO_STATE_ERROR;
			}
...
int vproRunState(int state, void* stimulus, void* data)
{
	...
	case VP_STIM_READINGS:
		...
		{
			MsgLog(PRI_MEDIUM, "vproRunState: retrying archive record from console:");
			MsgLog(PRI_MEDIUM, "vproRunState: you may need to cycle power on the console"
				"(including batteries) to resolve this condition.");
			if (vpifWakeupConsole(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproRunState: ARC WAKEUP failed");
				return VPRO_STATE_RUN;
			}

			if (vpifSendDumpAfterRqst(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproRunState: DMPAFT_RQST failed");
				return VPRO_STATE_ERROR;
			}
		...
		else
		{
			...
			if (vpifWakeupConsole(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproRunState: LOOP WAKEUP failed");
				return VPRO_STATE_RUN;
			}

			if (vpifSendLoopRqst(work, 1) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproRunState: LOOP_RQST failed");
				return VPRO_STATE_ERROR;
			}
	...
	case VP_STIM_ARCHIVE:
		...
		if (vpifWakeupConsole(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproRunState: ARC WAKEUP failed: retrying...");
			radUtilsSleep(1000);
			if (vpifWakeupConsole(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproRunState: 2nd ARC WAKEUP failed!");
				return VPRO_STATE_RUN;
			}
		}
...
int vproDumpAfterState(int state, void* stimulus, void* data)
{
	...
	case STIM_TIMER:
		...
		if (vpifWakeupConsole(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproDumpAfterState: WAKEUP failed");
			return VPRO_STATE_RUN;
		}

		if (vpifSendDumpAfterRqst(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproDumpAfterState: DMPAFT_RQST failed");
			return VPRO_STATE_ERROR;
		}
	...
	case STIM_IO:
		...
		*/
		...
		{
			MsgLog(PRI_HIGH, "vproDumpAfterState: DMP_DATETIME failed");
			return VPRO_STATE_ERROR;
		}
...
int vproDumpAfterAckState(int state, void* stimulus, void* data)
{
	...
	case STIM_TIMER:
		...
		{
			MsgLog(PRI_HIGH, "vproDumpAfterAckState: CANCEL failed");
			return VPRO_STATE_ERROR;
		}

		if (vpifWakeupConsole(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproDumpAfterAckState: WAKEUP failed");
			return VPRO_STATE_RUN;
		}

		if (vpifSendDumpDateTimeRqst(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproDumpAfterAckState: DMP_DATETIME failed");
			return VPRO_STATE_ERROR;
		}
	...
	case STIM_IO:
		...
		*/
		...
		{
			radProcessTimerStart(work->ifTimer, WVD_READ_RECOVER_INTERVAL);
			MsgLog(PRI_HIGH, "vproDumpAfterAckState: station read failed");
			return VPRO_STATE_READ_RECOVER;
		}
		...
		if (((VP_IF_DATA*)(work->stationData))->archivePages > 0)
		{
			...
			{
				MsgLog(PRI_HIGH, "vproDumpAfterAckState: ACK failed");
				return VPRO_STATE_ERROR;
			}
			...
			radProcessTimerStart(work->ifTimer, VP_RESPONSE_TIMEOUT(work->stationIsWLIP));
#if 0
			MsgLog(PRI_STATUS, "downloading %d pages from Vantage Pro...",
		...
		else
		{
			...
			if (((VP_IF_DATA*)(work->stationData))->doLoopFlag)
			{
				...
				if (vpifWakeupConsole(work) == ERROR)
				{
					MsgLog(PRI_HIGH, "vproDumpAfterAckState (initial): WAKEUP failed");
					return VPRO_STATE_ERROR;
				}

				if (vpifSendLoopRqst(work, 1) == ERROR)
				{
					MsgLog(PRI_HIGH, "vproDumpAfterAckState: LOOP_RQST failed");
					return VPRO_STATE_ERROR;
				}
...
int vproReceiveArchiveState(int state, void* stimulus, void* data)
{
	...
	case STIM_TIMER:
		// serial IF timer expiry
		MsgLog(PRI_HIGH, "vproReceiveArchiveState: timed out waiting for archive page from VP console!");

		if (vpifSendCancel(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproReceiveArchiveState: CANCEL failed");
			return VPRO_STATE_ERROR;
		}
		...
		if (!work->runningFlag)
		{
			...
			if (vpifWakeupConsole(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproReceiveArchiveState (initial): WAKEUP failed");
				return VPRO_STATE_ERROR;
			}

			if (vpifSendLoopRqst(work, 1) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproReceiveArchiveState: LOOP_RQST failed");
				return VPRO_STATE_ERROR;
			}
	...
	case STIM_IO:
		...
		if (recsRX == ERROR)
		{
			...
			MsgLog(PRI_HIGH, "vproReceiveArchiveState: read archive page failed");
		...
		{
#if 0
			MsgLog(PRI_STATUS, "... %d pages done",
				((VP_IF_DATA*)(work->stationData))->archivePages);
#endif
			...
			if (vpifWakeupConsole(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproReceiveArchiveState: WAKEUP1 failed");
				return VPRO_STATE_RUN;
			}
...
int vproLoopState(int state, void* stimulus, void* data)
{
	...
	case STIM_TIMER:
		...
		if (vpifWakeupConsole(work) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproLoopState: WAKEUP failed");
			return VPRO_STATE_RUN;
		}

		if (vpifSendLoopRqst(work, 1) == ERROR)
		{
			MsgLog(PRI_HIGH, "vproLoopState: LOOP_RQST failed");
			return VPRO_STATE_ERROR;
		}
	...
	case STIM_IO:
		...
		if (((VP_IF_DATA*)(work->stationData))->doArchiveFlag)
		{
			...
			if (vpifSendDumpAfterRqst(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "vproLoopState: DMPAFT_RQST failed");
				return VPRO_STATE_ERROR;
			}
			radProcessTimerStart(work->ifTimer, VP_RESPONSE_TIMEOUT(work->stationIsWLIP));
			return VPRO_STATE_DMPAFT_RQST;
