...
static int daemonStationInitComplete(void* eventData)
{
	...
	{
		// failed startup!
		MsgLog(PRI_HIGH, "daemonStationInitComplete: station startup failed!");
		return ERROR;
	}
	...
	if (!wviewdWork.runningFlag)
	{
		...
		{
			MsgLog(PRI_HIGH, "daemonStationInitComplete: stationGetPosition failed!");
			return ERROR;
		}
		...
		MsgLog(PRI_STATUS, "-- Station Init Complete --");
		...
		if ((int)wviewdWork.archiveDateTime == ERROR)
		{
			...
			MsgLog(PRI_STATUS, "no archive records found in database!");
		}
		else
		{
			MsgLog(PRI_STATUS, "newest archive record: %4.4d-%2.2d-%2.2d %2.2d:%2.2d",
...
static void daemonCheckArchiveRecord(ARCHIVE_PKT* newRecord)
{
	...
	{
		MsgLog(PRI_MEDIUM, "daemonCheckArchiveRecord: basic data values not changing!");
		NoChangeCounter = 0;
	}
...
static void daemonStoreArchiveRecord(ARCHIVE_PKT* newRecord)
{
	...
	if (newRecord == NULL)
	{
		MsgLog(PRI_MEDIUM, "daemonStoreArchiveRecord: record is NULL!");
		return;
	}
	...
	// push extra LOOP2 sensors in to the database
	if (wviewdWork.loopPkt.extraTemp[0] < 122 && wviewdWork.loopPkt.extraTemp[0]>14)
		newRecord->value[DATA_INDEX_extraTemp1] = wviewdWork.loopPkt.extraTemp[0];
	if (wviewdWork.loopPkt.extraTemp[1] < 122 && wviewdWork.loopPkt.extraTemp[1]>14)
		newRecord->value[DATA_INDEX_extraTemp2] = wviewdWork.loopPkt.extraTemp[1];
	if (wviewdWork.loopPkt.extraTemp[2] < 122 && wviewdWork.loopPkt.extraTemp[2]>14)
		newRecord->value[DATA_INDEX_extraTemp3] = wviewdWork.loopPkt.extraTemp[2];
	if (wviewdWork.loopPkt.windchill < 122 && wviewdWork.loopPkt.windchill >14)
		newRecord->value[DATA_INDEX_windchill] = wviewdWork.loopPkt.windchill;
	if (wviewdWork.loopPkt.heatindex < 122 && wviewdWork.loopPkt.heatindex >14)
		newRecord->value[DATA_INDEX_heatindex] = wviewdWork.loopPkt.heatindex;
	if (wviewdWork.loopPkt.dewpoint < 122 && wviewdWork.loopPkt.dewpoint >14)
		newRecord->value[DATA_INDEX_dewpoint] = wviewdWork.loopPkt.dewpoint;

	if (dbsqliteArchiveStoreRecord(newRecord) == ERROR)
	{
		MsgLog(PRI_MEDIUM, "daemonStoreArchiveRecord: dbsqliteArchiveStoreRecord failed!!!");
		return;
	}
...
static int daemonSysInit(WVIEWD_WORK* work)
{
	...
	*/
	...
	if (stat(temp, &fileData) != 0)
	{
		...
		{
			MsgLogInit(PROC_NAME_DAEMON, TRUE, TRUE);
			MsgLog(PRI_CATASTROPHIC,
...
static void defaultSigHandler(int signum)
{
	...
	case SIGHUP:
		...
		MsgLog(PRI_STATUS, "wviewd: SIGHUP - toggling log verbosity %s",
			((retVal == 0) ? "OFF" : "ON"));
	...
	case SIGSYS:
		...
		MsgLog(PRI_CATASTROPHIC, "wviewd: recv sig %d: shutting down!", signum);
		abort();
	...
	default:
		...
		MsgLog(PRI_HIGH, "wviewd: recv sig %d: exiting!", signum);
...
static void archiveTimerHandler(void* parm)
{
	...
	if (ntime < (wviewdWork.nextArchiveTime - 4))
	{
		...
		MsgLog(PRI_MEDIUM, "archiveTimerHandler: system time has skewed, adjusting...");
		stationStartArchiveTimerUniform(&wviewdWork);
		return;
	}
	...
	else
	{
		...
		{
			MsgLog(PRI_MEDIUM, "STATION: no new archive record generated "
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	{
		MsgLogInit(PROC_NAME_DAEMON, TRUE, TRUE);
		MsgLog(PRI_CATASTROPHIC, "system init failed!\n");
		MsgLogExit();
		exit(1);
	}
	...
	*/
	...
	{
		MsgLogInit(PROC_NAME_DAEMON, TRUE, TRUE);
		MsgLog(PRI_CATASTROPHIC, "radSystemInit failed!");
		MsgLogExit();
		exit(1);
	}
	...
	*/
	...
	if (pidfile == NULL)
	{
		MsgLog(PRI_CATASTROPHIC, "lock file create failed!\n");
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
		exit(1);
	}
	...
	MsgLog(PRI_STATUS, "%s starting ...", globalWviewVersionStr);
	...
	{
		MsgLog(PRI_CATASTROPHIC, "config database is missing!!!\n");
	...
	{
		wvconfigExit();
		MsgLog(PRI_CATASTROPHIC, "wvutilsSetVerbosity failed!");
		daemonSysExit(&wviewdWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
		exit(1);
	...
	if (iValue <= 0)
	{
		MsgLog(PRI_MEDIUM, "Rain Season Start Month not found - defaulting to 1 (JAN)...\n");
	...
	else
	{
		...
		{
			MsgLog(PRI_MEDIUM, "Invalid Rain Season Start Month %d found - defaulting to 1 (JAN)...\n",
				wviewdWork.stationRainSeasonStart);
			wviewdWork.stationRainSeasonStart = 1;
		}
		else
		{
			MsgLog(PRI_STATUS, "Rain Season Start Month set to %d\n",
				wviewdWork.stationRainSeasonStart);
		}
	}
	...
	else
	{
		...
		MsgLog(PRI_STATUS, "Rain Storm Start Trigger set to %5.2f in/hr\n",
	...
	else
	{
		...
		MsgLog(PRI_STATUS, "Rain Storm Stop Time set to %d hours\n",
			wviewdWork.stationRainStormIdleHours);
	}
	...
	else
	{
		...
		MsgLog(PRI_STATUS, "Rain YTD preset set to %.2f inches\n",
	...
	else
	{
		...
		MsgLog(PRI_STATUS, "ET YTD preset set to %.3f inches\n",
	...
	else
	{
		...
		{
			MsgLog(PRI_STATUS, "rain/ET YTD preset Year set to %d\n",
	...
	{
		MsgLog(PRI_STATUS, "station polling interval set to %d seconds",
	...
	if (wviewdWork.IsAlertEmailsEnabled)
	{
		...
		if (sValue == NULL)
		{
			MsgLog(PRI_HIGH, "NO alert email TO address given - disabling email alerts...");
		...
		if (sValue == NULL)
		{
			MsgLog(PRI_HIGH, "NO alert email FROM address given - disabling email alerts...");
			wviewdWork.IsAlertEmailsEnabled = 0;
		}
		else
		{
	...
	{
		MsgLog(PRI_HIGH, "statusInit failed - exiting...");
		daemonSysExit(&wviewdWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
		exit(1);
	...
	{
		MsgLog(PRI_HIGH, "dbsqliteArchiveInit failed");
	...
	{
		MsgLog(PRI_HIGH, "radTimerCreate failed");
	...
	{
		MsgLog(PRI_HIGH, "sync radTimerCreate failed");
		statusUpdateMessage("radTimerCreate failed");
		statusUpdate(STATUS_ERROR);
		radTimerDelete(wviewdWork.cdataTimer);
	...
	{
		MsgLog(PRI_HIGH, "radMsgRouterInit failed!");
	...
	{
		MsgLog(PRI_HIGH, "dbsqliteHiLowInit failed");
	...
	MsgLog(PRI_STATUS, "-- Station Init Start --");
	if (stationInit(&wviewdWork, daemonArchiveIndication) == ERROR)
	{
		MsgLog(PRI_HIGH, "stationInit failed!");
	...
	if (wviewdWork.medium.type == MEDIUM_TYPE_DEVICE)
	{
		if (radProcessIORegisterDescriptor(wviewdWork.medium.fd,
			...
			== ERROR)
		{
			MsgLog(PRI_HIGH, "IORegDescriptor failed");
	...
	MsgLog(PRI_STATUS, "exiting normally...");
