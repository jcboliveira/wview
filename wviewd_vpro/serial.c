...
static int serialInit(WVIEW_MEDIUM* med, char* deviceName)
{
	...
	if (med->fd == -1)
	{
		MsgLog(PRI_CATASTROPHIC, "Serial device %s failed to open: %s",
	...
	if (flock(med->fd, LOCK_EX) < 0)
	{
		if (errno == EOPNOTSUPP)
		{
			MsgLog(PRI_MEDIUM, "serial device locking not supported by OS for %s",
				deviceName);
		}
		else
		{
			MsgLog(PRI_CATASTROPHIC, "Serial device %s locked by other program!",
				deviceName);
			return ERROR;
		}
	}
...
static int serialRestart(WVIEW_MEDIUM* med)
{
	...
	serialExit(med);
	MsgLog(PRI_HIGH, "serialRestart: attempting restart");
	while ((!wviewdIsExiting()) && (serialInit(med, work->device) == ERROR))
	{
		MsgLog(PRI_HIGH, "serialRestart: restart failed");
		radUtilsSleep(5000);
		MsgLog(PRI_HIGH, "serialRestart: retrying restart");
	}
	if (!wviewdIsExiting())
	{
		MsgLog(PRI_HIGH, "serialRestart: recovered");
	}
	return OK;
}
