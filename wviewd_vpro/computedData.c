...
static int computeDataChanges(WVIEWD_WORK* work)
{
	...
	if (retVal != ERROR)
	{
		...
		if (current->dewpoint > 0 &&
			arcRecord.value[DATA_INDEX_dewpoint] > 0)
		{
			store->weekchangedewpt = current->dewpoint - (float)arcRecord.value[DATA_INDEX_dewpoint];
		}
		else
		{
			store->weekchangedewpt = 0;
		...
		if (current->extraTemp[0] > 0 &&
			arcRecord.value[DATA_INDEX_extraTemp1] > 0)
			store->weekchangeatemp = current->extraTemp[0] - (float)arcRecord.value[DATA_INDEX_extraTemp1];
		else
			store->weekchangeatemp = 0;

		if (current->extraTemp[1] > 0 &&
			arcRecord.value[DATA_INDEX_extraTemp2] > 0)
			store->weekchangeatemp1 = current->extraTemp[1] - (float)arcRecord.value[DATA_INDEX_extraTemp2];
		else
			store->weekchangeatemp1 = 0;

		if (current->soilTemp1 > ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_soilTemp1] > ARCHIVE_VALUE_NULL)
			store->weekchangesoiltemp1 = current->soilTemp1 - (float)arcRecord.value[DATA_INDEX_soilTemp1];
		else
			store->weekchangesoiltemp1 = 0;

		if (current->soilMoist1 >= ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_soilMoist1] > ARCHIVE_VALUE_NULL)
			store->weekchangesoilmoist1 = current->soilMoist1 - (int16_t)arcRecord.value[DATA_INDEX_soilMoist1];
		else
			store->weekchangesoilmoist1 = 0;

		if (current->leafWet1 >= ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_leafWet1] > ARCHIVE_VALUE_NULL)
			store->weekchangeleafwet1 = current->leafWet1 - (int16_t)arcRecord.value[DATA_INDEX_leafWet1];
		else
			store->weekchangeleafwet1 = 0;
	}
	...
	if (retVal != ERROR)
	{
		...
		if (current->dewpoint > 0 &&
			arcRecord.value[DATA_INDEX_dewpoint] > 0)
		{
			store->daychangedewpt = current->dewpoint - (float)arcRecord.value[DATA_INDEX_dewpoint];
		}
		else
		{
			store->daychangedewpt = 0;
		...
		if (current->extraTemp[0] > 0 &&
			arcRecord.value[DATA_INDEX_extraTemp1] > 0)
			store->daychangeatemp = current->extraTemp[0] - (float)arcRecord.value[DATA_INDEX_extraTemp1];
		else
			store->daychangeatemp = 0;

		if (current->extraTemp[1] > 0 &&
			arcRecord.value[DATA_INDEX_extraTemp2] > 0)
			store->daychangeatemp1 = current->extraTemp[1] - (float)arcRecord.value[DATA_INDEX_extraTemp2];
		else
			store->daychangeatemp1 = 0;

		if (current->soilTemp1 > ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_soilTemp1] > ARCHIVE_VALUE_NULL)
			store->daychangesoiltemp1 = current->soilTemp1 - (float)arcRecord.value[DATA_INDEX_soilTemp1];
		else
			store->daychangesoiltemp1 = 0;

		if (current->soilMoist1 >= ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_soilMoist1] > ARCHIVE_VALUE_NULL)
			store->daychangesoilmoist1 = current->soilMoist1 - (int16_t)arcRecord.value[DATA_INDEX_soilMoist1];
		else
			store->daychangesoilmoist1 = 0;

		if (current->leafWet1 >= ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_leafWet1] > ARCHIVE_VALUE_NULL)
			store->daychangeleafwet1 = current->leafWet1 - (int16_t)arcRecord.value[DATA_INDEX_leafWet1];
		else
			store->daychangeleafwet1 = 0;
	}
	...
	if (retVal != ERROR)
	{
		...
		if (current->dewpoint > 0 &&
			arcRecord.value[DATA_INDEX_dewpoint] > 0)
		{
			store->hourchangedewpt = current->dewpoint - (float)arcRecord.value[DATA_INDEX_dewpoint];
		}
		else
		{
			store->hourchangedewpt = 0;
		...
		if (current->extraTemp[0] > 0 &&
			arcRecord.value[DATA_INDEX_extraTemp1] > 0)
			store->hourchangeatemp = current->extraTemp[0] - (float)arcRecord.value[DATA_INDEX_extraTemp1];
		else
			store->hourchangeatemp = 0;

		if (current->extraTemp[1] > 0 &&
			arcRecord.value[DATA_INDEX_extraTemp2] > 0)
			store->hourchangeatemp1 = current->extraTemp[1] - (float)arcRecord.value[DATA_INDEX_extraTemp2];
		else
			store->hourchangeatemp1 = 0;

		if (current->soilTemp1 > ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_soilTemp1] > ARCHIVE_VALUE_NULL)
			store->hourchangesoiltemp1 = current->soilTemp1 - (float)arcRecord.value[DATA_INDEX_soilTemp1];
		else
			store->hourchangesoiltemp1 = 0;

		if (current->soilMoist1 >= ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_soilMoist1] > ARCHIVE_VALUE_NULL)
			store->hourchangesoilmoist1 = current->soilMoist1 - (int16_t)arcRecord.value[DATA_INDEX_soilMoist1];
		else
			store->hourchangesoilmoist1 = 0;

		if (current->leafWet1 >= ARCHIVE_VALUE_NULL &&
			arcRecord.value[DATA_INDEX_leafWet1] > ARCHIVE_VALUE_NULL)
			store->hourchangeleafwet1 = current->leafWet1 - (int16_t)arcRecord.value[DATA_INDEX_leafWet1];
		else
			store->hourchangeleafwet1 = 0;
	}
...
static int computeDataDay(WVIEWD_WORK* work, time_t lastTime)
{
	...
	{
		MsgLog(PRI_STATUS, "computeDataDay: %4.4d%2.2d%2.2d",
...
static int computeDataWeek(WVIEWD_WORK* work, time_t lastTime)
{
	...
	for (i = 1; (timenow < currentTime) && (i <= 7); i++)
	{
		...
		{
			numrecs += retVal;
			MsgLog(PRI_STATUS, "computeDataWeek: %4.4d%2.2d%2.2d",
				bkntimenow.tm_year + 1900,
				bkntimenow.tm_mon + 1,
				bkntimenow.tm_mday);
		}
...
static int computeDataMonth(WVIEWD_WORK* work, time_t lastTime)
{
	...
	MsgLog(PRI_STATUS, "computeDataMonth: %4.4d%2.2d",
...
static int computeDataYear(WVIEWD_WORK* work, time_t lastTime)
{
	...
	for (i = startMonth.tm_mon + 1; i <= nowmonth; i++)
	{
		...
		MsgLog(PRI_STATUS, "computeDataYear: %4.4d%2.2d", nowyear, i);
...
static int computeDataAllTime(WVIEWD_WORK* work, time_t firstTime)
{
		...
		i++)
	{
		...
		MsgLog(PRI_STATUS, "computeDataAllTime: %4.4d%2.2d", j, i);
...
{
	float       windAvg, tempAvg, humidAvg;
	...
	humidAvg = sensorGetAvg(&work->sensors.sensor[STF_HOUR][SENSOR_OUTHUMID]);
	work->loopPkt.intervalAvgWCHILL = sensorGetAvg(&work->sensors.sensor[STF_HOUR][SENSOR_WCHILL]);
...
static void intervalHousekeeping(WVIEWD_WORK* work)
{
	...
	humidAvg = sensorGetAvg(&work->sensors.sensor[STF_INTERVAL][SENSOR_OUTHUMID]);
	work->loopPkt.intervalAvgWCHILL = sensorGetAvg(&work->sensors.sensor[STF_INTERVAL][SENSOR_WCHILL]);
	work->loopPkt.intervalAvgWSPEEDF = windAvg;
...
int computedDataStoreSample(WVIEWD_WORK* work)
{
	...
	if (work->loopPkt.windchill !=ARCHIVE_VALUE_NULL) 
		sensorUpdate(&sample[SENSOR_WCHILL], work->loopPkt.windchill);
	if (work->loopPkt.heatindex !=ARCHIVE_VALUE_NULL) 
	...
	if (work->loopPkt.UV != ARCHIVE_VALUE_NULL)
		sensorUpdate(&sample[SENSOR_UV], (float)work->loopPkt.UV);
	if (work->loopPkt.radiation !=4000)
		sensorUpdate(&sample[SENSOR_SOLRAD], (float)work->loopPkt.radiation);
	if (work->loopPkt.extraTemp[0] !=ARCHIVE_VALUE_NULL)
		sensorUpdate(&sample[SENSOR_EXTRATEMP1], work->loopPkt.extraTemp[0]);
	if (work->loopPkt.extraTemp[1] !=ARCHIVE_VALUE_NULL)
		sensorUpdate(&sample[SENSOR_EXTRATEMP2], work->loopPkt.extraTemp[1]);
	if (work->loopPkt.extraTemp[2] !=ARCHIVE_VALUE_NULL)
		sensorUpdate(&sample[SENSOR_EXTRATEMP3], work->loopPkt.extraTemp[2]);
	if (work->loopPkt.soilTemp1 !=ARCHIVE_VALUE_NULL)
		sensorUpdate(&sample[SENSOR_SOILTEMP1], work->loopPkt.soilTemp1);
	if (work->loopPkt.soilMoist1 !=254)
		sensorUpdate(&sample[SENSOR_SOILMOIST1], work->loopPkt.soilMoist1);
	if (work->loopPkt.leafWet1 != 254)
		sensorUpdate(&sample[SENSOR_LEAFWET1], work->loopPkt.leafWet1);
...
ARCHIVE_PKT* computedDataGenerateArchive(WVIEWD_WORK* work)
{
	...
	if (sensorGetSamples(&sample[SENSOR_OUTTEMP]) == 0)
	{
		...
		MsgLog(PRI_MEDIUM, "computedDataGenerateArchive: no samples for this interval!");
	...
	if (work->loopPkt.dewpoint != ARCHIVE_VALUE_NULL)
		ArcRecStore.value[DATA_INDEX_dewpoint] = work->loopPkt.dewpoint;

	if (work->loopPkt.windchill != ARCHIVE_VALUE_NULL)
		ArcRecStore.value[DATA_INDEX_windchill] = work->loopPkt.windchill;

	if (work->loopPkt.heatindex != ARCHIVE_VALUE_NULL)
		ArcRecStore.value[DATA_INDEX_heatindex] = work->loopPkt.heatindex;
...
int computedDataInit(WVIEWD_WORK* work)
{
	...
	MsgLog(PRI_STATUS, "initializing computed data values...");
