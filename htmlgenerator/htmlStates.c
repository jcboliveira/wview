...
static void processNewArchiveRecord(HTML_WORK* work, WVIEW_MSG_ARCHIVE_NOTIFY* armsg)
{
	...
	{
		MsgLog(PRI_STATUS, "DST change: updating astronomical times for new local time...");
	...
	if (work->LastArchiveDateTime != 0)
	{
		...
		if (deltaArchiveIntervals < work->archiveInterval)
		{
			...
			MsgLog(PRI_MEDIUM, "processNewArchiveRecord: "
		...
		<= 0)
	{
		...
		MsgLog(PRI_MEDIUM, "no data found for sample...");
	}
	...
	if (work->histLastHour != currHour)
	{
			...
			<= 0)
		{
			...
			MsgLog(PRI_MEDIUM, "no data found for last hour...");
		}
	...
	if (work->histLastDay != currDay)
	{
			...
			<= 0)
		{
			...
			MsgLog(PRI_MEDIUM, "no data found for last day...");
		}
		else
		{
...
static int requestDataPackets(HTML_WORK* work)
{
	...
	{
		MsgLog(PRI_HIGH, "requestDataPackets: radMsgRouterMessageSend failed!");
		return ERROR;
	}
...
int htmlIdleState(int state, void* stimulus, void* data)
{
	...
	case STIM_DUMMY:
		...
		if (radMsgRouterMessageSend(WVIEW_MSG_TYPE_REQUEST, &msg, sizeof(msg)) == ERROR)
		{
			MsgLog(PRI_HIGH, "htmlIdleState: radMsgRouterMessageSend failed!");
...
int htmlStationInfoState(int state, void* stimulus, void* data)
{
	...
	case STIM_QMSG:
		if (stim->msgType == WVIEW_MSG_TYPE_STATION_INFO)
		{
			...
			if (work->mgrId == NULL)
			{
				MsgLog(PRI_HIGH, "htlmgrInit failed!");
			...
			MsgLog(PRI_STATUS, "initializing barometric pressure trend");
			if (htmlmgrBPTrendInit(work->mgrId, work->timerInterval / 60000)
				== ERROR)
			{
				MsgLog(PRI_HIGH, "htmlmgrBPTrendInit failed!");
			...
			MsgLog(PRI_STATUS, "initializing historical stores (this may take some time...)");
			if (htmlmgrHistoryInit(work->mgrId) == ERROR)
			{
				MsgLog(PRI_HIGH, "htmlStationInfoState: htmlmgrHistoryInit failed!");
			...
			{
				MsgLog(PRI_CATASTROPHIC, "dbsqliteHiLowInit failed!");
				statusUpdateMessage("dbsqliteHiLowInit failed!");
				statusUpdate(STATUS_ERROR);
				return HTML_STATE_ERROR;
			}

			MsgLog(PRI_STATUS, "NOAA: initializing Database");

			// initialize the NOAA database:
			if (dbsqliteNOAAInit() == ERROR)
			{
				MsgLog(PRI_CATASTROPHIC, "dbsqliteNOAAInit failed!");
			...
			{
				MsgLog(PRI_HIGH, "htmlStationInfoState: requestDataPackets failed!");
			...
			*/
			...
			MsgLog(PRI_STATUS, "doing initial html generation now...");
			...
			{
				MsgLog(PRI_CATASTROPHIC, "indicator file create failed!");
...
int htmlRunState(int state, void* stimulus, void* data)
{
	...
	case STIM_TIMER:
		if (stim->timerNumber == TIMER_GENERATE)
		{
			...
			if (requestDataPackets(work) == ERROR)
			{
				MsgLog(PRI_HIGH, "htmlRunState: requestDataPackets failed!");
				statusUpdateMessage("requestDataPackets failed!");
				statusUpdate(STATUS_ERROR);
				return HTML_STATE_ERROR;
			}
...
int htmlDataState(int state, void* stimulus, void* data)
{
	...
	case STIM_TIMER:
		if (stim->timerNumber == TIMER_RX_PACKETS)
		{
			...
			// Raise a stink:
			MsgLog(PRI_HIGH, "wviewd NOT responding!");
			MsgLog(PRI_HIGH, "Are there remnants of a previous failed start?");
			MsgLog(PRI_HIGH, "Continuing - if this is not an isolated event you should investigate further.");
			return state;
		}
...
int htmlErrorState(int state, void* stimulus, void* data)
{
	...
	case STIM_QMSG:
	case STIM_TIMER:
		MsgLog(PRI_STATUS, "htmlErrorState: received stimulus %d", stim->type);
		break;
	}
