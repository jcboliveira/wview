...
static int htmlSysInit(HTML_WORK* work)
{
	...
	*/
	...
	{
		MsgLogInit(PROC_NAME_HTML, FALSE, TRUE);
		MsgLog(PRI_CATASTROPHIC, "wviewd process not running - aborting!");
		MsgLogExit();
		return ERROR;
	}
	...
	*/
	...
	{
		MsgLogInit(PROC_NAME_HTML, FALSE, TRUE);
		MsgLog(PRI_CATASTROPHIC, "HTML template directory %s is missing...aborting!",
			devPath);
		MsgLogExit();
		return ERROR;
	}
	...
	*/
	...
	{
		MsgLogInit(PROC_NAME_HTML, FALSE, TRUE);
		MsgLog(PRI_CATASTROPHIC, "lock file %s exists, older copy may be running - aborting!",
			work->pidFile);
		MsgLogExit();
		return ERROR;
	}
	...
	*/
	...
	if (stat(devPath, &fileData) != 0)
	{
		...
		{
			MsgLogInit(PROC_NAME_HTML, FALSE, TRUE);
			MsgLog(PRI_CATASTROPHIC, "Cannot create device dir: %s...",
				devPath);
			MsgLogExit();
			return ERROR;
		}
...
static void defaultSigHandler(int signum)
{
	...
	case SIGHUP:
		...
		MsgLog(PRI_STATUS, "htmlgend: SIGHUP - toggling log verbosity %s",
			((retVal == 0) ? "OFF" : "ON"));
		MsgLog(PRI_STATUS, "htmlgend: SIGHUP - "
	...
	default:
		...
		MsgLog(PRI_CATASTROPHIC, "htmlgend: recv signal %d: exiting!", signum);
...
)
{
	...
	{
		MsgLog(PRI_HIGH, "htmlgend: received shutdown from wviewd");
		htmlWork.exiting = TRUE;
		return;
	}
...
/*
modificado, chamar diretamente db update
*/
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	{
		MsgLogInit(PROC_NAME_HTML, TRUE, TRUE);
	...
	*/
	...
	{
		MsgLog(PRI_CATASTROPHIC, "wvconfigInit failed!");
	...
	{
		wvconfigExit();
		MsgLog(PRI_CATASTROPHIC, "htmlgend daemon is NOT enabled - exiting...");
		htmlSysExit(&htmlWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
		exit(1);
	...
	{
		MsgLog(PRI_STATUS, "!! configured for metric units/conversion !!");
		...
		if (wvutilsGetRainIsMM())
			MsgLog(PRI_STATUS, "!! Rain units will be mm !!");
		else
			MsgLog(PRI_STATUS, "!! Rain units will be cm !!");
	}
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_STATUS, "Set your default radar image - using default");
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_STATUS, "Set your default forecast URL - using default");
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_STATUS, "Set your station name - using 'WVIEW_STATION'");
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_STATUS, "Set your station city - using 'WVIEW_CITY'");
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_STATUS, "Set your station state - using 'WVIEW_STATE'");
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_CATASTROPHIC, "HTMLGEN_IMAGE_PATH not found!");
		wvconfigExit();
		htmlSysExit(&htmlWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
	...
	else
	{
		...
		MsgLog(PRI_STATUS, "generating to %s", htmlWork.imagePath);
	}

	sValue = wvconfigGetStringValue(configItem_HTMLGEN_HTML_PATH);
	if (sValue == NULL)
	{
		MsgLog(PRI_CATASTROPHIC, "HTMLGEN_HTML_PATH not found!");
		wvconfigExit();
		htmlSysExit(&htmlWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
	...
	else
	{
		...
		MsgLog(PRI_STATUS, "templates at %s", htmlWork.htmlPath);
	}

	iValue = wvconfigGetINTValue(configItem_HTMLGEN_START_OFFSET);
	if (iValue < 0)
	{
		MsgLog(PRI_CATASTROPHIC, "HTMLGEN_START_OFFSET not found - defaulting to 0");
	...
	if (iValue <= 0)
	{
		MsgLog(PRI_CATASTROPHIC, "HTMLGEN_GENERATE_INTERVAL not found - defaulting to 1");
	...
	if (iValue >= 0)
	{
		...
		if (htmlWork.isDualUnits)
			MsgLog(PRI_STATUS, "!! Dual units will be displayed !!");
	}
	else
	{
	...
	if (sValue == NULL)
	{
		MsgLog(PRI_MEDIUM, "HTMLGEN_WIND_UNITS not found, defaulting to MPH...");
	...
	else if (!strncmp(sValue, "mph", 3))
	{
		...
		htmlWork.windUnits = HTML_WINDUNITS_MPH;
		wvutilsSetWindUnits(HTML_WINDUNITS_MPH);
		MsgLog(PRI_STATUS, "Using mph as default wind units");
	}
	else if (!strncmp(sValue, "m/s", 3))
	{
		...
		MsgLog(PRI_STATUS, "Using m/s as default wind units");
	}
	else if (!strncmp(sValue, "knots", 5))
	{
		...
		MsgLog(PRI_STATUS, "Using knots as default wind units");
	}
	else if (!strncmp(sValue, "km/h", 4))
	{
		...
		MsgLog(PRI_STATUS, "Using km/h as default wind units");
	}
	else
	{
		// default to mph:
		htmlWork.windUnits = HTML_WINDUNITS_MPH;
		wvutilsSetWindUnits(HTML_WINDUNITS_MPH);
		MsgLog(PRI_STATUS, "DB value not recognized: using mph as default wind units");
	}
	...
	{
		MsgLog(PRI_HIGH, "statusInit failed");
		htmlSysExit(&htmlWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
		exit(1);
	...
	if (htmlWork.stateMachine == NULL)
	{
		...
		MsgLog(PRI_HIGH, "radStatesInit failed");
		htmlSysExit(&htmlWork);
		radProcessExit();
		radSystemExit(WVIEW_SYSTEM_ID);
		exit(1);
	...
	if (radStatesAddHandler(htmlWork.stateMachine, HTML_STATE_IDLE,
		...
		== ERROR)
	{
		MsgLog(PRI_HIGH, "radStatesAddHandler failed");
	...
	{
		MsgLog(PRI_HIGH, "radTimerCreate 2 failed");
	...
	{
		MsgLog(PRI_HIGH, "radTimerCreate 3 failed");
