...
void sensorAddSample(WV_SENSOR* sensor, WV_SENSOR* sample, int DoSensorDbg)
{
	if (DoSensorDbg)
	{
		MsgLog(PRI_HIGH, "sensorAddSample: Low %f@%2.2d:%2.2d  High %f@%2.2d:%2.2d",
	...
	if (sample->low <= ARCHIVE_VALUE_NULL || sample->high <= ARCHIVE_VALUE_NULL)
	{
		if (DoSensorDbg)
		{
			MsgLog(PRI_HIGH, "sensorAddSample: invalid high or low!");
		}
		return;
	}
	...
	if (sensor->low > sample->low)
	{
		if (DoSensorDbg)
		{
			MsgLog(PRI_HIGH, "sensorAddSample: New Low: %f@%2.2d:%2.2d",
				sample->low, wvutilsGetHour(sample->time_low), wvutilsGetMin(sample->time_low));
		}
	...
	if (sensor->high < sample->high)
	{
		if (DoSensorDbg)
		{
			MsgLog(PRI_HIGH, "sensorAddSample: New High: %f@%2.2d:%2.2d",
				sample->high, wvutilsGetHour(sample->time_high), wvutilsGetMin(sample->time_high));
		}
...
char* sensorGetLowTime(WV_SENSOR* sensor, char* store)
{
	...
	else
	{
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR LOWTIME DBG: "
		...
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR LOWTIME RESULT DBG: %s", store);
		}
	}
	return store;
}
...
char* sensorGetLowDate(WV_SENSOR* sensor, char* store, char* dateFormat)
{
	...
	else
	{
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR LOWDATE DBG: "
				"low: %.2f, time_low: %u, high: %.2f, time_high: %u,"
				"samples: %d",
				sensor->low,
				(unsigned int)sensor->time_low,
		...
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR LOWDATE RESULT DBG: %s", store);
		}
	}
	return store;
}
...
char* sensorGetHighTime(WV_SENSOR* sensor, char* store)
{
	...
	else
	{
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR HIGHTIME DBG: "
				"low: %.2f, time_low: %u, high: %.2f, time_high: %u,"
				"samples: %d",
				sensor->low,
				(unsigned int)sensor->time_low,
		...
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR HIGHTIME RESULT DBG: %s", store);
		}
	}
	return store;
}
...
char* sensorGetHighDate(WV_SENSOR* sensor, char* store, char* dateFormat)
{
	...
	else
	{
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR HIGHDATE DBG: "
				"low: %.2f, time_low: %u, high: %.2f, time_high: %u,"
				"samples: %d",
				sensor->low,
				(unsigned int)sensor->time_low,
		...
		if (sensor->debug)
		{
			MsgLog(PRI_MEDIUM, "SENSOR HIGHDATE RESULT DBG: %s", store);
		}
	}
	return store;
}
